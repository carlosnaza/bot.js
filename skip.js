...
(function() {
  ...
  pupOnline = function() {
    ...
    myname = me.username;
    currentversion = "1.0";
    log("Você Acaba de Ativa o BOT" + currentversion + " do [SUB] #Carlostyfler");
    return API.sendChat("Não tenha medo, " + myname + " Está aqui para salva você!");
  };
      ...
      if (timeSinceLastActivity > data.afkTime) {
        if (user.getIsDj()) {
          ...
          if (user.getWarningCount() === 0) {
            user.warn();
            _results.push(API.sendChat("@" + user.getUser().username + ", Você não digitou nada no bater papo em cerca de 1 hora. Você está AFK? Se você não mostrar atividade em 4 minutos eu vou retirá-lo da cabine."));
            ...
            if (timeSinceLastWarning > twoMinutes) {
              ...
              if (DJs.length > 0 && DJs[0].id !== user.getUser().id) {
                API.sendChat("@" + user.getUser().username + ", Eu já avisei uma vez. Por favor, fique ativo.");
  ...
  afksCommand = (function(_super) {
    ...
    afksCommand.prototype.functionality = function() {
      ...
      if (msg === '') {
        return API.sendChat("Ninguém parece estar AFK!");
      } else {
  ...
  statusCommand = (function(_super) {
    ...
    statusCommand.prototype.functionality = function() {
      ...
      totals = '' + t.songs + ' Teve: ' + t.woots + ' woots, ' + t.mehs + ' :+1:, and ' + t.curates + ' :heart:.'
      msg = launch + totals;
      return API.sendChat(msg);
    };
  ...
  staffCommand = (function(_super) {
    ...
    staffCommand.prototype.staff = function() {
      ...
      if (stringstaff.length === 0) {
        stringstaff = "Não tem Mods ou Adm ativo nessa Porra !";
      }
      return stringstaff;
    };
