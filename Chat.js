...
var fbMsg = ["Entrem na Pagina da Sala: https://www.facebook.com/Depressao.Friendzone", "Grupo da sala: https://www.facebook.com/groups/friendzonegroup/"];
var rulesMsg = "Regras: 1) Video no Maximo 8 minutos. 2) Sem Flood! 3) Nao escrever em colorido. 4)Respeitar os Adms. 5) Nao Fiquem Pedindo Cargos.";
...
function initUIListeners() {
	...
	$("#plug-btn-face").on("click", function() {
		if (clicked == false) {
			clicked = true;
			clickTimer = setInterval("checkClicked();", 631);
...
function overPlayedSongs(data) {
	if (overPlayed.indexOf(Models.room.data.media.id) > -1) {
		API.sendChat("/me auto skip ligado, Musica Repetida. Coloque outra!");
		...
		setTimeout("new RoomPropsService(document.location.href.split('/')[3],false,true,1,5);", 631);
	}
	if (Models.room.data.media.duration > 481) {
		API.sendChat("/me auto skip ligado, música acima do limite permetido, coloque outra.");
		setTimeout("new RoomPropsService(document.location.href.split('/')[3],true,true,1,5);", 300);
		setTimeout("new ModerationForceSkipService;", 600);
		setTimeout("new RoomPropsService(document.location.href.split('/')[3],false,true,1,5);", 631);
	}
...
$('#plugbot-css').remove();
$('#plugbot-js').remove();
$('#chat-messages').append('<div class="chat-update"><span class="chat-text">Bem vindo ao auto-skip editado pelo Carlos Victor. </span></div>');
